...
def pca_recommendation_viz(df, scale_data=True, top_n_pc=3):
    ...
    # 1. Scree plot
    axes[0].plot(
        range(1, len(explained_var) + 1), explained_var * 100, "o-", label="Individual"
    )
    ...
    if strong_pcs == 0:
        recommendation = (
            "Keep existing features (PCA offers little dimensionality reduction)."
        )
        ...
        recommendation = (
            "Keep existing features (original features carry distributed information)."
        )
